...
void interlude_render_dialog_before(const ActData *act, int vw, int vh) {
    ...
    // Objectif
    DrawText("OBJECTIF :", px, py, 10, (Color){80,160,80,255});
    py += 14;
    char obj_buf[80];
    clip_text(act->objective.description, iw, 12, obj_buf, sizeof(obj_buf));
    DrawText(obj_buf, px + M_S, py, 12, (Color){150,220,150,255});
    py += 20;
    ...
    // On affiche le dialogue ligne par ligne
    {
        ...
        while (d[di] && dy < cy + ph/2 - 40) {
            ...
            if (li > 0) {
                ...
                DrawText(tbuf, px, dy, 10, (Color){168,148,102,255});
            }
            dy += 13;
        }
    }
                                   ...
                                   int scrap_earned, int vw, int vh)
{
    ...
    if (act->unlock_msg) {
        DrawText(act->unlock_msg, px, py, 11,
